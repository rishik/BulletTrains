...
class BlockSender implements java.io.Closeable, FSConstants {
   ...
   */
  private static final int MIN_BUFFER_WITH_TRANSFERTO = 16*1024;
  ...
  private int sendChunks(ByteBuffer pkt, int maxChunks, OutputStream out) 
    ...
    //ClientTraceLog.info(String.format("BLOCKSENDER_RISHI PacketLength %d CHECKSUMLEN %d", packetLen, checksumLen));
    byte[] buf = pkt.array();
...
//      ClientTraceLog.info(String.format("BLOCKSENDER_RISHI WHAT NORMAL TRANSFER %d", len));
...
//          ClientTraceLog.info(String.format("BLOCKSENDER_RISHI socket out suspicious %d", len));
          sockOut.write(buf, 0, dataOff + len);
        } else {
          //first write the packet
          sockOut.write(buf, 0, dataOff);
//          ClientTraceLog.info(String.format("BLOCKSENDER_RISHI socket out test dataoffset %d length %d", len, dataOff));
...
//    ClientTraceLog.info(String.format("BLOCKSENDER_RISHI %d", BUFFER_SIZE));
    return len;
  }
...
//        ClientTraceLog.info(String.format("BLOCKSENDER_RISHI BUFFER_SIZE %d", BUFFER_SIZE));
      ...
      if (clientTraceFmt != null) {
        ...
        ClientTraceLog.info(String.format("RISHI %d %d", startTime, endTime));
      }
      close();
    }
